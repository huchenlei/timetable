...
var cur;
var currentlist;
var convert_day = { "mon": 1,"tue": 2,"wed": 3,"thu": 4,"fri": 5};
...
var over_lap = function(a, b) {
	for (var i = 0; i < a.length; i++) 
		for (var j = 0; j < b.length; j++) 
			if (a[i].day == b[j].day && 
				Math.max(a[i].start/3600, b[j].start/3600) < Math.min(a[i].end/3600, b[j].end/3600))
				return true;
	return false;
};
    ...
    $.get(url, function(result) {
      ...
      for (i = 0; i < result.courses.length; i ++) {
        ...
        var new_tr = document.createElement('tr');
            ...
            new_div.innerHTML = session.code;
...
$(document).ready(function(){
    ...
    $('#normal-search').click(function() {
      var course_code = $('#search-by-code').val();
      ...
      $.getJSON(url, function(result) {
        ...
        for (i = 0; i < len; i++) {
          ...
          if (len2 > 0) {
            for (n = 0; n < len2; n++) {
              course_object = {
                code: classItem.sections[n].code,
              ...
              newList.innerHTML = classItem.sections[n].code;
